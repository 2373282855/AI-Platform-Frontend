  ...
  <div>
    ...
    <span class="file-tips">
      <slot name="tip">
        支持{{ acceptTypes }}
        <div></div>
        文件大小不能超过{{ props.fileSize / 1024 }}G；最多上传{{ props.limit }}个；
      </slot>
    </span>
...
import { NoticeSuccess, NoticeError, MsgWarning, MsgError } from "@/utils/koi.ts";
...
const props = withDefaults(defineProps<IUploadFilesProps>(), {
  acceptType: ".apk,.app,.aab,.zip,.xls,.xlsx,.pdf,.doc,.docx,.txt,.jar, .ipa",
  acceptTypes: "[.apk,.app,.aab,.zip,.xls,.xlsx,.pdf,.doc,.docx,.txt,.jar, .ipa]",
  isMultiple: true,
  limit: 1,
  disabled: false,
  fileSize: 10240,
    ...
    koi
      ...
      .then((res: any) => {
        ...
        fileList.value.push({
          ...
          url: import.meta.env.VITE_SERVER + res.data.file_url
        });
        ...
        NoticeSuccess(fileMap.filename +"，文件上传成功🌻");
      })
      .catch(error => {

...
// @ts-ignore
import { NoticeSuccess, NoticeError, MsgError, MsgWarning, MsgBox, MsgInfo } from "@/utils/koi.ts";
...
const handleSwitch = (row: any) => {
  ...
  MsgBox("确认要[" + text + "]-[" + row.deptName + "]部门吗？")
    .then(async () => {
...
const handleDelete = (row: any) => {
  ...
  MsgBox("您确认需要删除部门名称[" + row.deptName + "]么？")
    .then(async () => {
      try {
        await deleteById(id);
        NoticeSuccess("删除成功🌻");

...
const tableList = ref<any>([
  ...
  {
    noticeId: 2,
    noticeTitle: "维护通知：2023-11-23 ADMIN将持续进行维护",
    noticeType: "2",
    noticeContent: "维护内容",
    noticeStatus: "0",
    sorted: 2,
  ...
  {
    noticeId: 1,
    noticeTitle: "温馨提醒：2023-11-23 ADMIN生日",
    noticeType: "1",
    noticeContent: "新版本内容",
    noticeStatus: "0",
    sorted: 1,
...
const handleSwitch = (row: any) => {
  ...
  MsgBox("确认要[" + text + "]-[" + row.noticeTitle + "]吗？")
    .then(async () => {
...
const handleDelete = (row: any) => {
  ...
  MsgBox("您确认需要删除公告名称[" + row.noticeTitle + "]么？")
    .then(async () => {
      try {
        await deleteById(id);
        handleTableData();

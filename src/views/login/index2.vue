  ...
  <div class="bigBox">
    <div class="box" ref="box" v-if="!isMobile">
      ...
      <div class="slide-box" ref="slideRef">
        <h1 class="slide-title" v-if="switchLock">欢迎登录-BeautyPlus AI效率🌻</h1>
        <h1 class="slide-title" v-if="!switchLock">欢迎注册-BeautyPlus AI效率🌻</h1>
...
import { MsgError, NoticeSuccess, MsgSuccess } from "@/utils/koi.ts";
...
const handleKoiLogin = () => {
  ...
  (loginFormRef.value as any).validate(async (valid: any, fields: any) => {
    ...
    if (valid) {
      ...
      try {
        // 1、执行登录接口
        // const res: any = await koiLogin({ username, password, securityCode });
        // console.log("tokenValue", res.data.tokenValue);
        // userStore.setToken(res.data.tokenValue);
        userStore.setToken(authLogin.data.tokenValue, authLogin.data.tokenValue);
